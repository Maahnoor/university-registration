...
<label for="cars">Filter by Courses:</label>

<select name="f_id" id="cars">
	<?php while ($record1= mysqli_fetch_array($result)) { ?>
  <option value="<?php echo $record1['degree_id']?>"><?php echo $record1['course_name']?></option>
  <?php }?>
</select>
...
<label><?php echo $record1['student_name']?></label>
<?php } 	
}?>
</div>
